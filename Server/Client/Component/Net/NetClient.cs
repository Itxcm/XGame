...
namespace XClient
{
    [AddComponentMenu("XClient/Net/NetClient")]
    internal class NetClient : MonoSingleton<NetClient>
    {
        ...
        private const int DEF_POLL_INTERVAL_MILLISECONDS = 100; //default network thread hold on interval
        private const int DEF_TRY_CONNECT_TIMES = 3;            //default the number of retries the server
        private const int DEF_RECV_BUFFER_SIZE = 64 * 1024;     //default initial buffer size of recvStream
        private const int DEF_PACKAGE_HEADER_LENGTH = 4;        //default package header size
        private const int DEF_SEND_PING_INTERVAL = 30;          //default send ping package interval
        private const int NetConnectTimeout = 10000;    //default connect wait milliseconds
        private const int DEF_LOAD_WHEEL_MILLISECONDS = 1000;   //default wait some milliseconds then show load wheel
        private const int NetReconnectPeriod = 10;              //default reconnect seconds
