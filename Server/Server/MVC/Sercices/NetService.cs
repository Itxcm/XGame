...
namespace MVC //TODO 这里用到了配置的变量 不可以加额外的命名空间
{
    internal class NetService
    {
        private static TcpSocketListener ServerListener;

        public bool Init(int port)
        {
            ServerListener = new TcpSocketListener(IPAddress.Any, Server.Properties.Settings.Default.ServerPort, 10);
